...
class StableDiffusionBase:
    ...
    def __init__(
        ...
        self.active_mobile_decoder = False
        if os.path.exists(mdecoder_path):
            self.active_mobile_decoder = True
            self.decoder_interpreter = tf.lite.Interpreter(model_path=mdecoder_path)
    ...
    def mdecoder(self, latent):
        self.decoder_interpreter.set_tensor(self.decoder_input_details[0]['index'],
                                            tf.reshape(latent, self.decoder_input_details[0]['shape']) )
        self.decoder_interpreter.invoke()
        return self.decoder_interpreter.get_tensor(self.decoder_output_details[0]['index'])
    ...
    def generate_image(
        ...
        # Decoding stage
        if self.active_mobile_decoder:
            decoded = self.mdecoder(latent) * 255
        else:
